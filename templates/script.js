  ...
  async function processSubscription() {
    ...
    try {
      ...
      localStorage.setItem('pending_subscription_ref', initData.reference);
  ...
  function analyzeFrame() {
    ...
    if (!IS_PREMIUM && USAGE_STATS.scans_remaining !== null && USAGE_STATS.scans_remaining <= 0) {
      if (confirm("You've reached your free scan limit (5/month). Upgrade to Premium for unlimited scans?")) {
  ...
  async function initiateDownload() {
    ...
    if (USAGE_STATS.downloads_remaining !== null && USAGE_STATS.downloads_remaining > 0) {
      if (confirm(`You have ${USAGE_STATS.downloads_remaining} free download(s) remaining. Use one now?`)) {
        downloadPDF();
  ...
  async function processPayment() {
    ...
    try {
      ...
      localStorage.setItem('pending_payment_ref', initData.reference);
      localStorage.setItem('pending_report', JSON.stringify(latestReport));
  ...
  async function verifyPayment(reference) {
    try {
      ...
      if (verifyData.status === "ok" && verifyData.can_download) {
        ...
        if (savedReport) {
          latestReport = JSON.parse(savedReport);
        }
  ...
  async function checkPaymentCallback() {
    ...
    if (paymentRef) {
      ...
      const pendingSubRef = localStorage.getItem('pending_subscription_ref');
      
      try {
        if (pendingSubRef === paymentRef) {
          await verifySubscription(paymentRef);
          localStorage.removeItem('pending_subscription_ref');
        } else {
